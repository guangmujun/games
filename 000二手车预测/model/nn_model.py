...
for train_index, vali_index in kfold:
    ...
    model.fit(k_x_train, k_y_train, batch_size=512, epochs=200, validation_data=(
        k_x_vali, k_y_vali), callbacks=[reduce_lr], verbose=2)  # callbacks=callbacks,
    oof_nn[vali_index] = model.predict(k_x_vali).reshape(
        (model.predict(k_x_vali).shape[0],))
    predictions_nn += model.predict(x_test).reshape(
        (model.predict(x_test).shape[0],)) / kfolder.n_splits
    predictions_train_nn += model.predict(x).reshape(
        (model.predict(x).shape[0],)) / kfolder.n_splits
